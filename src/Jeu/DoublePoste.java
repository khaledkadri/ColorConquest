/**
 * 
 * JPanel for a two-player networked game.
 * Each player chooses colors to fill an 8x8 grid.
 * Actions are synchronized between client and server.
 *
 * Author: Khaled KADRI
 * License: MIT
 */
...
import Connection.Envoi;
import Connection.Reception;
import Connection.Serveur;


public class DoublePoste extends JPanel implements ActionListener{
	...
	Reception reception;
	Envoi envoi;
	public boolean envoiAutorise=true;
	...
	public DoublePoste(String IP){
		...
		JLabel label1 = new JLabel("Two players via network");
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(serveur.moiserveur)
	    	Evenement(couleur,true);
	    else
	    	Evenement(couleur,false);
		
		/*******************************************************************************/

		// Check if the network game has not been initialized yet
		if(!start){

		    // If this instance is the server
		    if(serveur.moiserveur){
		        // Start a new thread to handle incoming messages from the client
		        Thread trecep = new Thread(new Reception(serveur.in, this, serveur));
		        trecep.start();

		        // Initialize the sending mechanism using the server's output stream
		        envoi = new Envoi(serveur.out);

		        // Stop the server listening thread (no longer needed)
		        tserveur.stop();
		    }
		    // If this instance is the client
		    else{
		        // Stop the server listening thread (probably inherited from previous logic)
		        tserveur.stop();

		        // Create a client socket and connect to the server
		        client = new Client();
		        client.creationClient(ip);

		        // Start a new thread to handle incoming messages from the server
		        Thread trecep = new Thread(new Reception(client.in, this, serveur));
		        trecep.start();

		        // Initialize the sending mechanism using the client's output stream
		        envoi = new Envoi(client.out);
		    }

		    // Mark the network as started to prevent re-initialization
		    start = true;
		}

		// Send the chosen color to the other player
		envoi.envoyer(couleur);
	...
	ArrayList<coordonees> bandej1(boolean[][] matricej){
	    ArrayList<coordonees> bordure = new ArrayList<coordonees>(); // List of border coordinates
	    ArrayList<Integer> indice = new ArrayList<Integer>(); // Temporary list of row indices
	    ...
	    for(int j = 0 ; j < 8 ; j++){
	        ...
	        // If the top cell of the column is not colored
	        if(matricej[i][j]==false){
	            for(i = 0 ; i < 8 ; i++)
	                if(matricej[i][j]) // Add any already-colored cells
	                    indice.add(i);
	        }
	        // If the top cell is already colored
	        else{
	            while(i<8 && matricej[i][j]==true)
	                i++;
	            indice.add(i-1); // Add the last colored cell index
	        }

	        // Add surrounding cells to the border list
	            ...
	            if(i>=0 && matricej[i][j]==true)
	                for(int i1 = i-1 ; i1 <= i+1 ; i1++)
	                    for(int j1 = j-1 ; j1 <= j+1 ; j1++){
	                        ...
	                        // Ensure cell is valid and not already in the list
	                        if(i1 !=-1 && j1 !=-1 && i1<8 && j1<8)
	                            if(!matricej[i1][j1] && !bordure.contains(cord))
	                                bordure.add(cord);
	                    }
	        }
	    }
	    return bordure; // Return all "border" cells around player 1's colored cells
	}
	...
	public static void main(String[] args) {
		DoublePoste jeu = new DoublePoste("");
